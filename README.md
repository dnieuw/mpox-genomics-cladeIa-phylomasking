# Site masking for MPXV cladeIa phylogenetics

This repository is modified from a similar [repository](https://github.com/WHO-Collaboratory/collaboratory-mpox-genomics-phylomasking) managed by the [community of practice on specialized surveillance for emergency response](https://github.com/WHO-Collaboratory/collaboratory-mpox-genomics-community).

## Files in this repository

In this repository, we maintain a list of the genomic positions that we recommend to mask when reconstructing phylogenetic trees for Clade Ia of  the monkeypox virus (MPXV). The rationale behind site masking is discussed [below](#why-do-sites-need-to-be-masked-in-MPXV-phylogenetics)

We currently maintain a list of sites for clade Ia named clade_Ia_mask.csv.

Sites within these masking files use the coordinates of [PX667572.1](https://www.ncbi.nlm.nih.gov/nuccore/PX667572.1)

The masking files are in a format that is compatible with [squirrel](https://github.com/aineniamh/squirrel). To use these files with squirrel, when running an alignment add the --additional-mask flag followed by the masking file. For example, to mask the recommended sites within clade Ia when aligning a set of sequences in a file called clade_Ia_unaligned.fasta, run:

```
squirrel --clade cladei clade_Ia_unaligned.fasta --additional-mask clade_Ia_mask.csv
```
...
* The site(s) that are suggested to be masked - please use coordinates relative to the PX667572.1 reference genome
